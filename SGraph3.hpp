...
public:
	...
	long int giantComponent(Vertex source);
...
long int Graph::giantComponent(Vertex source){
    bool visited[n];
    ...
    while(!searchComplete){
    	...
    	// -1 indicated all nodes have been visited already.
    	if(index == -1){
    		searchComplete = true;
    	}
    }
...
// if u is less than v, then attempt to form an edge. Otherwise, an attempt has already
// been made.
void Graph::formGiantComponent(Graph &g, int p){
	...
	for(Vertex i = 0; i < n; i++){
		for(Vsize j = 0; j < nlist[i].Size(); j++){
			chance = rand()%10000;
			// Don't attempt to delete the same edge twice. Cannot use name[i] if reading
			// graph from a file.
			if(i < nlist[i][j]){
				if(chance >= p){
					//delete edge
...
// Creates a graph with N nodes, broken up into groups. If the avgdegre is 1 more than the average in-deg
// you control the chance the one edge connecting to outside of the group will actually form
